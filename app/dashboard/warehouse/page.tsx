...
import { Package, Clock, UserCheck, AlertTriangle, Box } from 'lucide-react';
...
import { WarehouseBranchFilter } from '@/components/warehouse/branch-filter';
import { returnLoan, approveLoan } from '@/lib/actions/loans';
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

export default async function WarehousePage(props: { searchParams: Promise<{ branchId?: string }> }) {
        ...
        <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                    <WarehouseBranchFilter branches={branches} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="bg-blue-50/20 border-blue-100">
                    <CardHeader className="py-4 flex flex-row items-center justify-between space-y-0 text-blue-800">
                ...
                <Card className="bg-amber-50/20 border-amber-100">
                    <CardHeader className="py-4 flex flex-row items-center justify-between space-y-0 text-amber-800">
                ...
                <Card className="bg-red-50/20 border-red-100">
                    <CardHeader className="py-4 flex flex-row items-center justify-between space-y-0 text-red-800">
                ...
                <Card className="bg-emerald-50/20 border-emerald-100">
                    <CardHeader className="py-4 flex flex-row items-center justify-between space-y-0 text-emerald-800">
                    ...
                    <CardContent>
                        ...
                        {branchId && <p className="text-[10px] text-emerald-600 mt-1 uppercase font-semibold">En sede seleccionada</p>}
                    </CardContent>
                </Card>
            </div>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2">
                                                    {loan.status === 'REQUESTED' && (
                                                        <form action={async () => { 'use server'; await approveLoan(loan.id); }}>
                                    ...
                                    {loans.length === 0 && (
                                        <TableRow>
                                            <TableCell colSpan={6} className="text-center py-12 text-zinc-400">
                                                No hay préstamos registrados {branchId ? "para esta sede" : ""}.
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                            ...
                            <Table>
                                <TableHeader className="bg-zinc-50">
                                    <TableRow>
                                        ...
                                        <TableHead>Sede Actual</TableHead>
                                ...
                                <TableBody>
                                    {warehouseAssets.map((asset) => (
                                        <TableRow key={asset.id}>
                                            ...
                                            <TableCell className="text-sm">{new Date(asset.createdAt).toLocaleDateString()}</TableCell>
                                            <TableCell>
                                    ...
                                    {warehouseAssets.length === 0 && (
                                        <TableRow>
                                            <TableCell colSpan={5} className="text-center py-12 text-zinc-400">
                                                No hay herramientas disponibles en el almacén de esta sede.
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
